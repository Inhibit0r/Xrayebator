...
graph TB
    subgraph "Компьютер пользователя"
        subgraph "Renderer Process (Chromium)"
            UI["🖥️ Premium UI<br/>React + TypeScript + Framer Motion"]
            I18N["🌐 i18n<br/>RU / EN / ZH"]
        end

        subgraph "Main Process (Node.js)"
            IPC["📨 IPC Bridge<br/>contextBridge"]
            SSH["🔐 SSH Client<br/>ssh2 + TOFU"]
            DEPLOY["🚀 Deployer<br/>7-step workflow"]
            SUB["📋 Subscription Parser<br/>vless:// decoder"]
            STORE["💾 Server Store<br/>electron-store + keytar"]
            UPDATE["🔄 Auto Updater<br/>electron-updater"]
        end
    end

    subgraph "VPS (только скрипты)"
        SCRIPTS["📜 install.sh<br/>xrayebator quickstart"]
        XRAY["⚡ Xray Core<br/>(работает как демон)"]
        SUBSCRIPTION["📡 /sub/&lt;token&gt;<br/>HTTP endpoint"]
    end
    ...
    IPC --> SUB
    IPC --> STORE
    SSH -->|"SFTP + exec"| SCRIPTS
    SUB -->|"HTTP GET"| SUBSCRIPTION
    UPDATE -->|"GitHub Releases"| GH["📦 GitHub"]
```
...
sequenceDiagram
    ...
    participant R as 🖥️ Renderer (React)
    participant M as ⚙️ Main (Node.js)
    participant V as ☁️ VPS

    Note over U,V: Развёртка нового сервера
    U->>R: IP, порт, пароль, email
    R->>M: ipc: deploy:start
    M->>V: SSH connect
    M->>R: deploy:step(1, "SSH ✅")
    M->>V: SFTP upload install.sh + xrayebator
    M->>R: deploy:step(3, "Загрузка ✅")
    M->>V: bash install.sh (streaming stdout)
    M-->>R: deploy:log("Installing Xray core...")
    M-->>R: deploy:log("Configuring TLS...")
    M->>V: xrayebator quickstart --email ...
    V->>M: JSON {ok, subscription_url}
    M->>R: deploy:done({subscription_url})
    R->>M: subscription:fetch(url)
    M->>V: HTTP GET /sub/token
    V->>M: vless://... lines
    M->>R: subscription:result([keys])
    R->>U: Ключи + QR + "Скопировать"

    Note over U,V: Обновление ключей (позже)
    U->>R: Кнопка "🔄 Обновить"
    R->>M: subscription:refresh(server_id)
    M->>V: HTTP GET /sub/token
    V->>M: vless://...
    M->>R: subscription:result([keys])
    R->>U: Обновлённые ключи
```
