...
1. HAPP устарел — обновитесь до версии `3.3.6` или новее, полностью закройте все старые процессы
   HAPP, запустите ровно один актуальный экземпляр и обновите подписку.
2. Зелёный ping маршрута не подтверждает работу основного TUN: HAPP проверяет маршруты отдельным
   временным Xray-core. На Linux выполните `ss -lntp | grep ':10808'`; пустой вывод означает, что
   основной core не слушает порт, поэтому полностью перезапустите HAPP.
