...
`keytar` указан в зависимостях, но GUI пока не использует keychain для SSH-паролей: пароль вводится
на каждую операцию. Не рассчитывайте на приложение как на парольный сейф, пока это не изменится.
