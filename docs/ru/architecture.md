...
- HAPP получает plain-text список `vless://`, HAPP-заголовки и по умолчанию управляемый профиль
  `happ://routing/onadd/...`;
- профиль скачивает geo-базы через защищённые токеном `/sub/<token>/geoip.dat` и
  `/sub/<token>/geosite.dat`, поэтому клиенту не нужен прямой доступ к GitHub;
...
Для отключения задайте `HAPP_ROUTING_ENABLED=false` в `.happ_defaults.env`. Кастомный
`.happ_routing.json` заменяет управляемый профиль только после строгой проверки схемы HAPP;
невалидный JSON откатывается на управляемый профиль, а причина пишется в journal сервиса.
