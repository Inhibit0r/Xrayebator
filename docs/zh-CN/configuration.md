...
| `8` | 将单个配置档升级到 PQ XHTTP |
| `9` | HAPP 订阅：7 条线路的配置档、public TLS、链接、二维码、吊销 |
| `11` | 分流路由：域名直连，绕过 VPN |
| `12` | 级联与上游节点 |
| `13` | 自有域名与 self-steal 挡板 |
| `14` | 部署出站服务器，使本 VPS 成为级联的境外节点 |
| `0` | 退出 |

编号存在跳号 —— 当前版本没有第 `5`、`6`、`7`、`10` 项。
...
菜单第 `12` 项把参数保存到 `/usr/local/etc/xray/upstreams/cascade.json`，
...
第 `14` 项配置的是另一侧：把当前 VPS 变成境外节点，供另一台服务器的级联连入。
